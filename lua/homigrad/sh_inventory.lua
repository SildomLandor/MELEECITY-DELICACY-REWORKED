...
--
		...
		function DScrollPanel:Paint(w, h)
			...
			if #gridChildren <= 0 then
				statusText = "Empty"
			elseif searchEndTime >= CurTime() then
				statusText = txt
			end
			draw.DrawText(statusText, "ZCity_Small", w / 2, h / 2.8, Color(255,255,255,15), TEXT_ALIGN_CENTER)
		end
		local count2 = 0
		...
		for tab, things in pairs(inv) do
			...
			for k, i in ipairs(keys) do
				...
				button.Paint = function(self, w, h)
					...
					surface.SetDrawColor(button.col1, button.col1, button.col1, 15)

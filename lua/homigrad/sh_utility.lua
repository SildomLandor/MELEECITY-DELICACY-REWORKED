...
--\\ custom spawn
	...
	hook.Add("player_spawn", "homigrad-spawn3", function(data)
		...
		timer.Simple(0, function()
			...
			ply:SetSlowWalkSpeed(55)
			ply:SetLadderClimbSpeed(150)
			ply:SetCrouchedWalkSpeed(55)
...
--\\ Custom movement
	...
	--//
	if SERVER then
		local landSound = "leaden_sky/land.wav"
		local landSoundCooldown = 0.7
		local landSoundLevel = 60
		hook.Add("OnPlayerHitGround", "hg_landing_sound", function(ply, inWater, onFloater, speed)
			if not IsValid(ply) then return end
			if inWater or onFloater then return end
			local nextTime = ply.hg_landSoundNext or 0
			local now = CurTime()
			if nextTime > now then return end
			ply.hg_landSoundNext = now + landSoundCooldown
			sound.Play(landSound, ply:GetPos(), landSoundLevel, math.random(95, 105), 0.7)
		end)
	end
	...
	hook.Add("SetupMove", "HG(StartCommand)", function(ply, mv, cmd)
		...
		local movement_penalty = math.abs(sm * 1.35)
		...
		*/
		...
		if org.noradrenaline and org.noradrenaline > 0 then
			local noradrenalineBonus = 200 * math.Round(org.noradrenaline, 1)
			speed = speed + noradrenalineBonus
			inertia_len = inertia_len + noradrenalineBonus
		end
...
--\\ custom footsteps
	...
	local new_footsteps = {
		concrete = {"player/footsteps/concrete1.wav", "player/footsteps/concrete2.wav", "player/footsteps/concrete3.wav", "player/footsteps/concrete4.wav"},
		dirt = {"player/footsteps/dirt1.wav", "player/footsteps/dirt2.wav", "player/footsteps/dirt3.wav", "player/footsteps/dirt4.wav"},
		grass = {"player/footsteps/grass1.wav", "player/footsteps/grass2.wav", "player/footsteps/grass3.wav", "player/footsteps/grass4.wav"},
		metal = {"player/footsteps/metal1.wav", "player/footsteps/metal2.wav", "player/footsteps/metal3.wav", "player/footsteps/metal4.wav"},
		tile = {"player/footsteps/tile1.wav", "player/footsteps/tile2.wav", "player/footsteps/tile3.wav", "player/footsteps/tile4.wav"},
		...
		if SERVER then
			...
			if !(ply:IsWalking() or ply:Crouching()) and ent == ply then
				...
				if ply.PlayerClassName == "furry" then
					...
					local found = false
					for name, snd_table in pairs(new_footsteps) do
						if string.find(sound, name) then
							snd = snd_table[math.random(#snd_table)]
							found = true
							break
						end
					end
...
--\\ Remove Sandbox Jump Boost
	hook.Add("PlayerSpawn", "RemoveSandboxJumpBoost", function(ply)
		...
		PLAYER.SlowWalkSpeed		= 90		-- How fast to move when slow-walking (+WALK)
		...
		PLAYER.UnDuckSpeed			= 0.25		-- How fast to go from ducking, to not ducking
